...
and (mcc.path like (select catpath from vars) or mcc.path like (select catpath2 from vars2))
...
and (mc1.fullname not ilike '%sandbox%' and mc1.fullname not ilike '%sandpit%' and mc1.fullname not ilike '%sand pit%')
and (mcc4.name not ilike '%sandbox%' and mcc4.name not ilike '%sandpit%')
group by mcc4.id
), 
modules as 
(
